@include('PartialAdmin.SidebarNavbar')

<div class="content-body">
    <div class="container-fluid align-items-center">
        <div class="row">
            <div class="col-xl-12 col-xxl-4">
                <a href="/Company">
                    <div class="card">
                        <div class="card-body">
                            <div class="row align-items-center">
                                <div class="col-xl-12">
                                    <div class="d-flex align-items-center justify-content-center">
                                        <img class="img-responsive" src="images/perusahaan.svg" alt="">
                                    </div>
                                    <div class="text-center mt-4">
                                        <strong>
                                            <h4>Perusahaan</h4>
                                        </strong>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </a>
            </div>
            <div class="col-xl-12 col-xxl-4">
                <a href="/PayrollSalarySlip">
                    <div class="card">
                        <div class="card-body">
                            <div class="row align-items-center">
                                <div class="col-xl-12">
                                    <div class="d-flex align-items-center justify-content-center">
                                        <img class="img-responsive" src="images/payroll2.svg" alt="">
                                    </div>
                                    <div class="text-center mt-4">
                                        <strong>
                                            <h4>Payroll</h4>
                                        </strong>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </a>
            </div>
            <div class="col-xl-12 col-xxl-4">
                <a href="/WorkSchedule">
                    <div class="card">
                        <div class="card-body">
                            <div class="row align-items-center">
                                <div class="col-xl-12">
                                    <div class="d-flex align-items-center justify-content-center">
                                        <img class="img-responsive" src="images/presensi.svg" alt="">
                                    </div>
                                    <div class="text-center mt-4">
                                        <strong>
                                            <h4>Presensi</h4>
                                        </strong>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </a>
            </div>
            <div class="col-xl-12 col-xxl-4">
                <a href="/Announcement">
                    <div class="card">
                        <div class="card-body">
                            <div class="row align-items-center">
                                <div class="col-xl-12">
                                    <div class="d-flex align-items-center justify-content-center">
                                        <img class="img-responsive" src="images/pengumuman.svg" alt="">
                                    </div>
                                    <div class="text-center mt-4">
                                        <strong>
                                            <h4>Pengumuman</h4>
                                        </strong>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </a>
            </div>
            <div class="col-xl-12 col-xxl-4">
                <a href="/AccountsUsers">
                    <div class="card">
                        <div class="card-body">
                            <div class="row align-items-center">
                                <div class="col-xl-12">
                                    <div class="d-flex align-items-center justify-content-center">
                                        <img class="img-responsive" src="images/akundanpengguna.svg" alt="">
                                    </div>
                                    <div class="text-center mt-4">
                                        <strong>
                                            <h4>Akun & Pengguna</h4>
                                        </strong>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </a>
            </div>
            <div class="col-xl-12 col-xxl-4">
                <a href="/Validation">
                    <div class="card">
                        <div class="card-body">
                            <div class="row align-items-center">
                                <div class="col-xl-12">
                                    <div class="d-flex align-items-center justify-content-center">
                                        <img class="img-responsive" src="images/foto5.png" alt="">
                                    </div>
                                    <div class="text-center mt-4">
                                        <strong>
                                            <h4>Validasi</h4>
                                        </strong>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </a>
            </div>
        </div>
    </div>
</div>

<div class="footer">
    <div class="copyright">
        <p>Copyright © Designed &amp; Developed by <a href="https://dexignlab.com/"
                target="_blank">Kalopsia</a> 2022</p>
    </div>
</div>
<script src="{{ asset('vendor/global/global.min.js') }}"></script>
<script src="{{ asset('vendor/chart.js/Chart.bundle.min.js') }}"></script>
<script src="{{ asset('vendor/jquery-nice-select/js/jquery.nice-select.min.js') }}"></script>

<!-- Apex Chart -->
<script src="{{ asset('vendor/apexchart/apexchart.js') }}"></script>
<script src="{{ asset('vendor/nouislider/nouislider.min.js') }}"></script>
<script src="{{ asset('vendor/wnumb/wNumb.js') }}"></script>

<!-- Dashboard 1 -->
<script src="{{ asset('js/dashboard/dashboard-1.js') }}"></script>

<script src="{{ asset('js/custom.min.js') }}"></script>
<script src="{{ asset('js/dlabnav-init.js') }}"></script>
<script src="{{ asset('js/demo.js') }}"></script>
<script src="{{ asset('js/styleSwitcher.js') }}"></script>

<!-- Apex Chart -->
<script src="{{ asset('vendor/apexchart/apexchart.js') }}"></script>

<script src="{{ asset('vendor/jquery-nice-select/js/jquery.nice-select.min.js') }}"></script>

<!-- Chart ChartJS plugin files -->
<script src="{{ asset('vendor/chart.js/Chart.bundle.min.js') }}"></script>
<script src="{{ asset('js/plugins-init/chartjs-init.js') }}"></script>
