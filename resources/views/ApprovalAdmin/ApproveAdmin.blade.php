...
<body>
    ...
    <div id="main-wrapper">
                ...
                <ul class="metismenu" id="menu">
                    <li class="dropdown header-profile">
                        <a class="nav-link" href="javascript:void(0);" role="button" data-bs-toggle="dropdown">
                            <img src="{{ asset('gambar/' . auth()->user()->foto) }}" width="20"
                                alt="" />
                    ...
                    <li class="red-hover"><a class="ai-icon hover-red" href="/ApprovalAdmin" aria-expanded="false">
                    ...
                    <li class="red-hover"><a class="ai-icon hover-red" href="/PermitLeaveAdmin"
                            aria-expanded="false">
                    ...
                    <li class="red-hover"><a class=" ai-icon hover-red" href="AdminReport" aria-expanded="false">
                            <svg class="svg" xmlns="http://www.w3.org/2000/svg" height="48"
                                viewBox="0 -960 960 960" width="48">
                                <path
                    ...
                    <li class="red-hover"><a class="has-arrow ai-icon hover-red" href="javascript:void()"
                            aria-expanded="false">
                            <svg class="svg" xmlns="http://www.w3.org/2000/svg" height="48"
                                viewBox="0 -960 960 960" width="48">
                                <path
                    ...
                    <li class="red-hover"><a class=" ai-icon hover-red" href="/EmployeeAdmin" aria-expanded="false">
                            <svg clss="svg" sxmlns="http://www.w3.org/2000/svg" height="48"
                                viewBox="0 -960 960 960" width="48">
                                <path
                    ...
                    <li class="red-hover"><a href="/StartPayroll" class="ai-icon hover-red" aria-expanded="false">
                            <svg class="svg" xmlns="http://www.w3.org/2000/svg" height="48"
                                viewBox="0 -960 960 960" width="48">
                                <path
        ...
        ***********************************-->
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                        ...
                                        <tbody class="text-center">
                                            @foreach ($approval as $data)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <span>
                                                            <a href="#" class="me-4" data-toggle="modal"
                                                                data-target="#exampleModalCenter" data-placement="top"
                                                                title="Terima"><i
                                                                    class="bi bi-person-check-fill text-success accept"
                                                                    data-id="{{ $data->id }}"
                                                                    data-user-nama="{{ $data->User->name }}"></i>
                                                            </a>
                                                            <a href="#" data-bs-toggle="modal"
                                                                data-bs-target="#exampleModal"
                                                                data-bs-id="{{ $data->id }}"
                                                                data-bs-user-nama="{{ $data->User->name }}"
                                                                data-bs-tooltip="tooltip" data-bs-placement="top"
                                                                title="Tolak">
                                                                <i class="bi bi-person-x-fill text-danger reject"></i>
                                                            </a>
                                                        </span>
                                                    </td>
                                                </tr>
                                ...
                                <div class="button-group">
                                    <button type="button" class="btn btn-success btn-xs">Konfirmasi Data yang
                                        Dipilih</button>
                                    <button type="button" class="btn btn-danger btn-xs">Tolak Data yang
                                        Dipilih</button>
                                </div>
                            </div>
                        </div>
        ...
        <form action="{{ url('save_notifikasi', ['id' => $data->id]) }}" method="POST" class="modal fade"
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="recipient-name" class="col-form-label"
                                style="font-weight: bold !important;">Untuk:</label>
                            <h2 id="recipient-heading">{{ $data->User->name }}</h2>
                        </div>
                        <div class="form-group">
                            <label for="reason" class="col-form-label">Reason:</label>
                            <textarea class="form-control @error('reason') is-invalid @enderror" id="editor" name="reason" cols="30"
                                rows="10"></textarea>
                            {{-- <div name="reason" id="summernote" class="form-control" cols="30" rows="10"></div> --}}
                            {{-- <input type="textarea" id="summernote" class="form-control" id="reason"
                                name="reason"> --}}
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-danger btn-xs" data-bs-dismiss="modal">Close</button>
                        <button type="submit" class="btn btn-success btn-xs">Send message</button>
                    </div>
                </div>
            </div>
        </form>
    ...
    ***********************************-->
    ...
    <script>
        // Ketika button diklik, ambil data dari atribut data-bs-id dan data-bs-user-nama dan tampilkan di modal
        document.querySelectorAll('[data-bs-toggle="modal"]').forEach((button) => {
            button.addEventListener('click', function() {
                const id = this.getAttribute('data-bs-id');
                const userNama = this.getAttribute('data-bs-user-nama');

                // Masukkan data ke dalam modal
                document.getElementById('recipient-name').value = userNama;
                document.getElementById('modal-data-id').innerText = id;
            });
        });
    </script>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Menggunakan class sebagai selector untuk tombol
        document.querySelectorAll('.btn-open-sweetalert').forEach(function(button) {
            button.addEventListener('click', function() {
                const id = this.getAttribute('data-id');
                const userNama = this.getAttribute('data-user-nama');

                // Tampilkan SweetAlert2 dengan data ID dan nama user
                Swal.fire({
                    title: 'Modal title',
                    html: `<p>ID: ${id}</p><p>Nama User: ${userNama}</p>`,
    ...
    $('.reject').click(function() {
        ...
        swal({
            ...
            if (inputValue) {
                ...
                const csrfToken = $('#csrf').text().trim();
                const id =$(this).data('id')

                // Menggunakan AJAX untuk mengirim data ke route yang ditentukan dalam route/web.php
                $.ajax({
                    type: "POST",
                    url: "save_notifikasi/"+id,

                    data: {
                        _token: csrfToken, // Sertakan CSRF token langsung dalam data
                        reason: alasanPenolakan,
                        is_read: false, // Atur sesuai kebutuhan, misalnya false karena notifikasi belum dibaca
                    },
                    success: function(response) {
                        // Redirect atau lakukan tindakan lain setelah data tersimpan
                        window.location = "/Rejected/" + id;

                        // Tampilkan notifikasi ke karyawan bahwa data telah ditolak
                        swal("Sukses! Data telah ditolak dengan alasan: " +
                            alasanPenolakan, {
                                icon: "success",
                            });
                    },
                    error: function() {
                        swal("Terjadi kesalahan saat mengirim data ke server.");
                    },
                });
            } else {
